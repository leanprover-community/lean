...
| (succ j) h := (decidable.lt_or_eq_of_le h).elim
...
by rw [← succ_pred_eq_of_pos h, pow_succ, nat.mul_zero]
...
local attribute [simp] nat.mul_comm
...
begin
  ...
  clear m,
  intros p IH,
  cases decidable.lt_or_le p (b^succ w) with h₁ h₁,
