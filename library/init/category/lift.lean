...
export has_monad_lift (monad_lift)
...
@[reducible] def has_monad_lift_to_has_coe {m n} [has_monad_lift m n] {α} : has_coe (m α) (n α) :=
⟨monad_lift⟩

instance has_monad_lift_refl (m) : has_monad_lift m m :=
⟨λ α, id⟩
...
export monad_functor (monad_map)

instance monad_functor_refl (m m') : monad_functor m m' m m' :=
⟨λ α f, f⟩
