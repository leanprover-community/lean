...
@[inline] def guard {f : Type u → Type v} [alternative f] (p : Prop) [decidable p] : f punit :=
if p then pure () else failure

@[inline] def assert {f : Type u → Type v} [alternative f] (p : Prop) [decidable p] : f (ulift (inhabited p)) :=
...
@[inline] def guardb {f : Type u → Type v} [alternative f] : bool → f punit
| tt := pure ()
| ff := failure
