...
1. A list of names identifying the case (`names`). This is a list of constructor
   names, one for each case split that was performed. For example, the sequence
   of tactics `cases n; cases xs`, where `n` is a natural number and `xs` is a
   list, will generate four cases tagged as follows:
   ...
   Note: In the case tag, these names are stored in reverse order. Thus, the
   `names` of the first case tag would be `list.nil, nat.zero`.
...
2. Information about the arguments introduced by the cases-like tactic.
   ...
   1. The `with_cases` tactic generates goals where the target quantifies over
      ...
      tag (since the target is a pi type). In addition to the `names`, it
   ...
   2. The `cases` and `induction` tactics do not add arguments to the target,
      ...
      this form are tagged with a `hyps` case tag. In addition to the `names`,
      it contains a list of *unique* names of the hypotheses that were
      introduced.
