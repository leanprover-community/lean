...
meta def is_prod_mk : expr → tactic (expr × expr)
| `(prod.mk %%a %%b) := return (a, b)
| _                  := failed
...
| tac :=
  ...
  else if t.is_napp_of `prod.lex 6 then
     let a := t.app_fn.app_arg in
     let b := t.app_arg in
     do (a₁, a₂) ← is_prod_mk a,
        (b₁, b₂) ← is_prod_mk b,
        (is_def_eq a₁ b₁ >> `[apply prod.lex.right] >> process_lex tac)
        <|>
        (`[apply prod.lex.left] >> tac)
  else
     tac
